...
import { ArrowLeft, RefreshCw, Mic, MicOff, Volume2, MessageCircle } from "lucide-react";
...
interface Message {
  role: "user" | "assistant";
  content: string;      // full content (for text mode)
  display?: string;     // progressively revealed (for voice mode)
}
...
export default function ChatPage() {
  ...
  const [emmaTyping, setEmmaTyping] = useState("");   // word-by-word reveal
  ...
  const speakWithTypewriter = useCallback((text: string, onDone?: () => void) => {
    ...
    let i = 0;
    const msPerWord = Math.max(180, Math.min(280, (text.length / words.length) * 40));
    ...
    const loadVoice = () => {
      ...
      const female =
        voices.find(v => v.lang.startsWith("en") && /samantha|karen|moira|victoria|zira|susan|female/i.test(v.name)) ??
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onend = () => {
      setUserStatus((prev) => {
        if (prev === "listening") {
          // grab final transcript from state then send
  ...
  // ── Call Emma API ─────────────────────────────────────────────
  async function callEmma(newMessages: Message[]) {
    ...
    try {
      ...
      const reply: string = data.reply ?? "💬 **Emma:** Sorry, I didn't catch that. Can you try again?\n\n📝 **Corrections:** –";
      const conv = extractConversation(reply);

      setUserStatus("idle");

      // Add message with empty display (will be filled by typewriter)
      setMessages((prev) => [...prev, { role: "assistant", content: reply, display: "" }]);

      speakWithTypewriter(conv, () => {
        // After Emma finishes, commit the full text and auto-listen
        setMessages((prev) =>
          prev.map((m, i) => i === prev.length - 1 ? { ...m, display: conv } : m)
        );
  ...
  function startConversation(chosenMode: "voice" | "text") {
    ...
    const openingText = opening[scenario];
    const firstMsg: Message = {
      role: "assistant",
      content: `💬 **Emma:** ${openingText}\n\n📝 **Corrections:** ✓`,
      display: "",
    };
          ...
          <div className="grid grid-cols-2 gap-3">
            {voiceSupported && (
              <button onClick={() => startConversation("voice")}
                className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-4 rounded-2xl flex flex-col items-center gap-2 transition-all active:scale-95">
                <Mic size={24} />
            ...
            <button onClick={() => startConversation("text")}
              className={cn("bg-primary-600 hover:bg-primary-700 text-white font-bold py-4 rounded-2xl flex flex-col items-center gap-2 transition-all active:scale-95",
          ...
          <div className="flex-1">
            ...
            <p className="text-xs font-medium text-green-500">
              {emmaStatus === "speaking" ? "● Speaking..." :
               userStatus === "listening" ? "● Listening to you..." :
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-44">
        <div className="max-w-lg mx-auto space-y-4">
          {messages.map((msg, i) => {
            ...
            const isEmmaLast = isLast && msg.role === "assistant" && emmaStatus === "speaking";
            return (
              <ChatBubble
                key={i}
                message={msg}
                liveText={isEmmaLast ? emmaTyping : undefined}
                isVoice={isVoice}
                onSpeak={(t) => speakWithTypewriter(t)}
              />
            );
          })}
          ...
          {userStatus === "listening" && (
            <div className="flex justify-end">
              <div className="bg-primary-100 border-2 border-primary-300 text-primary-800 rounded-2xl rounded-tr-sm px-4 py-3 max-w-[80%] text-sm">
                ...
                {liveTranscript || <span className="opacity-40 italic">Say something...</span>}
              </div>
            </div>
          )}
          ...
          {isVoice ? (
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-center">
                <button
                  onClick={() => {
                    if (userStatus === "listening") {
                      stopListening();
                    } else if (!isBusy) {
                      autoRestartRef.current = true;
                      startListening();
                    } else if (emmaStatus === "speaking") {
                      stopEverything();
                      setTimeout(startListening, 300);
                    }
                  }}
                  className={cn(
                    "w-20 h-20 rounded-full flex flex-col items-center justify-center gap-1 text-white font-semibold transition-all active:scale-95 shadow-lg",
              ...
              <p className="text-center text-xs text-gray-400">
                {userStatus === "listening" ? "I'm listening – tap to stop" :
                 emmaStatus === "speaking" ? "Emma is speaking – tap mic to interrupt" :
                 isBusy ? "Please wait..." :
                 "Tap to speak, conversation continues automatically"}
              </p>
              {/* Switch to text mode */}
              <button onClick={() => setMode("text")} className="w-full text-center text-xs text-gray-400 underline">
                Switch to text mode
              </button>
            </div>
          ) : (
            ...
            <div className="space-y-2">
              ...
              {voiceSupported && (
                <button onClick={() => setMode("voice")} className="w-full text-center text-xs text-purple-500 underline">
...
function ChatBubble({ message, liveText, isVoice, onSpeak }: {
  message: Message;
  liveText?: string;
  isVoice: boolean;
  ...
  const fullConv = convMatch ? convMatch[1].trim() : message.content;
  ...
  const showCorrections = !liveText && corrections && !isPerfect; // only show after done speaking
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="space-y-1.5 max-w-[85%]">
        <div className="bg-white rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm border border-gray-100 text-sm text-gray-800 leading-relaxed">
          ...
          {liveText !== undefined && <span className="inline-block w-0.5 h-4 bg-purple-400 animate-pulse ml-0.5 align-middle" />}
          {!liveText && !isVoice && (
            <button onClick={() => onSpeak(fullConv)} className="ml-1 text-gray-300 hover:text-primary-500 transition-colors inline-flex items-center" title="Listen">
              <Volume2 size={13} />
            </button>
          )}
        </div>
        {showCorrections && (
          <div className="bg-orange-50 text-orange-800 border border-orange-200 rounded-xl px-3 py-2 text-xs leading-relaxed">
            <span className="font-semibold">📝 </span>
            <span className="whitespace-pre-line">{corrections}</span>
          </div>
        )}
        {!liveText && isPerfect && corrections && (
          <div className="bg-green-50 text-green-700 border border-green-200 rounded-xl px-3 py-2 text-xs">
            ✓ Perfect English!
          </div>
        )}
      </div>
